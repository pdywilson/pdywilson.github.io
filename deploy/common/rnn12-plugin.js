CindyJS.registerPlugin(1, "rnn12", function(api) {
    ...
    async function getMelody(inputmelody, chords, cdycallback) {
        ...
        //design choice to set the next chord of the final note as the following chord in the chord progression
        chordsnew.push(getChord(chords[inputmelody[inputmelody.length-1][1]])); 
        chordsroot.push(getRoot(chords[inputmelody[inputmelody.length-1][1]])); 

        notesnew = notesnew.slice(-max_seq_len);
        chordsnew = chordsnew.slice(-max_seq_len-1);
        chordsroot = chordsroot.slice(-max_seq_len-1);
